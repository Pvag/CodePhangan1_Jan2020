// draws horizontal lines with different stroke weight - dynamic version!
//
...
void draw() {
  ...
  final int maxLineHeight = (height - 2*verticalSpacing)/(nLines - 1);
  int lineHeight = int(map(mouseX, 0, width, 0, maxLineHeight));
